...
const mt = {
    server: "moderntender.com",
    port: 443,
    api: "/wp-json/verification",
    token: "ynhHi2uY7%(7DZ&OrDMtL*h4"
}
...
function getMTTier (req, res) {
    ...
    if (req.query.phone) {
        ...
        var options = {
            hostname: mt.server,
            port: mt.port,
            path: mt.api + '/mt_tier?phone=' + req.query.phone,
            method: 'GET',
            headers: {
                'lsmtk': mt.token,
            },
        ...
        https.get(options, (resp) => {
            ...
            resp.on('end', () => {
                tier = JSON.parse(JSON.parse(data)).tier;
                respond(req, res, {tier})
            });


        }).on('error', (e) => {
            console.error(e);
            throw httpError(e, 401)
        });
    }

}

function setFundingSource (req, res, next) {
    if (req.body.phone && req.body.fundingSource) {
...
function setSSN (req, res, next) {
    if (req.body.phone && req.body.ssn) {
...
function getCustomerWithPhoneCode (req, res, next) {
    ...
    if (req.body.isMT) {
        ...
        var options = {
            hostname: mt.server,
            port: mt.port,
            path: mt.api + '/is_verified?phone_number='+phone,
            method: 'GET',
            headers: {
                'lsmtk': mt.token,
            },
        ...
        https.get(options, (resp) => {
            ...
            resp.on('end', () => {
                is_verified = JSON.parse(JSON.parse(data)).isVerified;
                tier = JSON.parse(JSON.parse(data)).tier;
                name = JSON.parse(JSON.parse(data)).mt_name;
                ...
                if (is_verified) {
                    ...
                    return pi.getPhoneCode(phone)
                        .then(code => {
                            return addOrUpdateCustomer(req)
                                .then(customer => respond(req, res, {code, customer, verified: is_verified, tier: tier, mt_name: name }))
                        })
                        .catch(err => {
                    ...
                    respond(req, res, {verified: false, tier: null, mt_name: null});
                }
            });
...
app.get('/mt_tier', getMTTier)
app.post('/funding_source', setFundingSource)
app.post('/ssn', setSSN)
